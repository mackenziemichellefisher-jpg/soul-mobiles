...
export default function ArchivePage() {
  ...
  useEffect(() => {
    const gifts = JSON.parse(localStorage.getItem('soulr_saved_gifts') || '[]');
    setSavedGifts(gifts.reverse()); // Show newest first
  }, []);

  const filteredGifts = filter === 'all' 
    ? savedGifts 
    : savedGifts.filter(gift => gift.type === filter);
    ...
    <div className="min-h-screen relative overflow-hidden" style={{
      ...
      <div className="relative z-10 flex flex-col min-h-screen p-4">
        <div className="text-center text-white mb-8">
          <h1 className="text-2xl font-bold mb-4">Your Archive</h1>
          <p className="text-white/80">All the gifts you've shared with the world</p>
        </div>

        {savedGifts.length === 0 ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center text-white">
              <p className="text-lg mb-4">No gifts yet</p>
              <button
